...
public class JSBundleImpl implements JSBundle {
	...
	public JSBundleImpl(JSFrameworkImpl framework, JSBundleData bundleData) {
		...
		parseSymbolicName((String) headers.get(Constants.BUNDLE_SYMBOLIC_NAME_HEADER));
		parseVersion((String) headers.get(Constants.BUNDLE_VERSION_HEADER));
		parseImports((String) headers.get(Constants.IMPORTS_HEADER));
		parseExports((String) headers.get(Constants.EXPORTS_HEADER));
		parseRequires((String) headers.get(Constants.REQUIRES_HEADER));
	}
